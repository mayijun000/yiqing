<template>
	<view>
		...
		<view class="text-xs" style="position:absolute;top:200upx;left:40upx;z-index:100000;color:#fff;">
			<text class="text-xxl text-bold">新冠肺炎小区查询</text>
		</view>
		<view class="text-xs" style="position:absolute;top:300upx;left:40upx;z-index:100000;color:#fff;">
		...
		<view class="cu-list menu">
			<view v-for="(itemObj,kindex) in communitylist" :key="kindex">
				<view v-for="(item,index) in itemObj" :key="index">
		...
		methods: {
			...
			openMap() {		
				try {
					...
					uni.navigateTo({
						url: '/pages/community/map?city=' + this.city + '&cityone=' + this.cityone
					})
				} catch (e) {
				    // error
				}
